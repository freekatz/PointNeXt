...
class LocalAggregation(nn.Module):
    ...
    def forward(self, pf, data) -> torch.Tensor:
...
class SetAbstraction(nn.Module):
    ...
    def forward(self, pf, data):
        ...
        if self.is_head:
            ...
            dp = dp - p.transpose(1, 2).unsqueeze(-1)  # (B, 3, npoint, nsample)
...
class PointNextEncoder(nn.Module):
    ...
    def _make_enc(self, layer_index, block, channels, blocks, stride, is_head=False):
        layers = []
        layers.append(SetAbstraction(layer_index, self.in_channels, channels,
        ...
        for i in range(1, blocks):
            layers.append(block(layer_index, self.in_channels,
