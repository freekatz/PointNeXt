...
from ..layers import create_convblock1d, create_convblock2d, create_act, \
    create_grouper, furthest_point_sample, random_sample, three_interpolation, create_mlp
import math
...
class LocalAggregation(nn.Module):
    ...
    def forward(self, pf, pe, data) -> torch.Tensor:
        ...
        if not is_head:
            ...
            for i in range(self.num_head):
                ...
                convsf.append(create_convblock2d(out_channels // (self.num_head), out_channels // (self.num_head),
                                                 norm_args=norm_args, act_args=act_args, **conv_args))
                convaug.append(
                    create_convblock2d(out_channels // (2 * self.num_head), out_channels // (2 * self.num_head),
            ...
            self.convfinal = create_convblock1d(out_channels, out_channels, norm_args=norm_args, act_args=act_args,
                                                **conv_args)
            ...
            self.conv_g1 = create_convblock1d(out_channels, out_channels, norm_args=norm_args, act_args=act_args,
                                              **conv_args)
...
class PointNATEncoder(nn.Module):
    ...
    def _make_enc(self, block, channels, i_th, blocks, stride, group_args, is_head=False):
        ...
        layers.append(SetAbstraction(i_th, self.in_channels, channels, i_th,
        ...
        for i in range(1, blocks):
            ...
            layers.append(block(i_th, self.in_channels,
    ...
    def forward(self, data):
        return self.forward_seg_feat(data)
